...
void battle(Player *player) {
    ...
    Supemon wild_supemon = create_supemon("Wild Supemon", 10 + rand() % 6, 1, 1, 2, 1, 2, "Griffe", "Grogne");
    ...
    while (player_supemon->hp > 0 && wild_supemon.hp > 0) {
        ...
        if (wild_supemon.hp > 0) {
            ...
            printf("%s sauvage utilise %s et inflige %d degats!\n", wild_supemon.name, wild_supemon.moves[0], damage);
        }
    }

    if (player_supemon->hp <= 0) {
        printf("Votre Supemon tombe! Vous perdez la bataille.\n");
    } else {
